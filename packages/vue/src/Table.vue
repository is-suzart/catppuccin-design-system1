    ...
    <table class="ctp-table">
        ...
        <tr>
          ...
          <th
            ...
            :class="[
              ...
              sortField === col.key ? 'ctp-table-th--active' : '',
              `ctp-table-cell--align-${col.align || 'left'}`,
            ]"
          ...
          >
            <div class="ctp-table-th-content">
              ...
              <span
                v-if="col.sortable"
                :class="['ctp-table-sort-icon', sortField === col.key ? 'ctp-table-sort-icon--active' : '']"
              >
      ...
      <tbody>
        <tr v-if="!isLoading && data.length === 0">
          <td :colspan="colspan" class="ctp-table-empty">
            {{ emptyState }}
          </td>
        </tr>
        ...
        <template v-if="isLoading">
          <template v-for="rIdx in loadingRowsCount" :key="'sk-' + rIdx">
            <tr v-if="rIdx === 1" class="ctp-table-header-spacer">
              <td :colspan="colspan">
            ...
            <tr
              :class="[
                ...
                rIdx === 1 ? 'ctp-table-tr--first' : '',
                rIdx === loadingRowsCount ? 'ctp-table-tr--last' : '',
              ]"
            >
              ...
              <td
                v-for="col in columns"
                :key="'sk-' + col.key"
                :class="col.align ? `ctp-table-cell--align-${col.align}` : ''"
              >
                <div
        ...
        <template v-else v-for="(row, idx) in sortedData" :key="rowKey(row)">
          <tr v-if="idx === 0" class="ctp-table-header-spacer">
            <td :colspan="colspan">
              <div class="ctp-table-header-spacer-inner" />
            </td>
          </tr>
          <tr
            :class="[
              isSelected(row) ? 'ctp-table-tr--selected' : '',
              idx === 0 ? 'ctp-table-tr--first' : '',
              idx === sortedData.length - 1 ? 'ctp-table-tr--last' : '',
            ]"
          >
