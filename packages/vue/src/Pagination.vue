<template>
  <nav :class="containerClass" aria-label="Pagination">
    <button
      class="ctp-pagination-item"
      :class="`ctp-pagination-item--${size}`"
    ...
    <template v-for="(item, idx) in range" :key="idx">
      <span v-if="item === DOTS" class="ctp-pagination-ellipsis">{{ DOTS }}</span>
      <button
        v-else
        class="ctp-pagination-item"
        :class="[
          `ctp-pagination-item--${size}`,
          { 'ctp-pagination-item--active': item === modelValue }
        ]"
...
const containerClass = computed(() => {
  return [
    'ctp-pagination',
    `ctp-pagination--${props.size}`,
    `ctp-pagination--${props.shape}`,
    `ctp-pagination--${props.color}`,
  ];
});
