    ...
    <table class="ctp-table">
      ...
      <tbody>
            ...
            >
              ...
              >
                <div v-if="colIdx === 0" style="display: flex; align-items: center; gap: 8px">
                  <div class="ctp-table-skeleton-bar" style="width: 16px; height: 16px; border-radius: 4px" />
                  <div class="ctp-table-skeleton-bar" style="width: 70%" />
                </div>
                <div
                  v-else
        ...
        <template v-else-if="visibleRows.length === 0">
          <tr>
            <td :colspan="columns.length + (showCheckbox ? 1 : 0)" class="ctp-table-empty">
              {{ emptyState }}
            </td>
          </tr>
        </template>
        <template v-else>
          <template v-for="({ row, depth, hasChildren, isExpanded }, rowIndex) in visibleRows" :key="rowKey(row)">
            <tr v-if="rowIndex === 0" class="ctp-table-header-spacer">
              <td :colspan="columns.length + (showCheckbox ? 1 : 0)">
                <div class="ctp-table-header-spacer-inner" />
              </td>
            </tr>
