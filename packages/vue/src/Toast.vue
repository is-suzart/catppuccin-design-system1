<template>
  <div
    ...
    :key="pos"
    :class="['ctp-toast-container', `ctp-toast-container--${pos}`]"
  >
    <div
      v-for="item in group"
      :key="item.id"
      :class="['ctp-toast', `ctp-toast--${item.variant}`, item.filled ? 'ctp-toast--filled' : '', item.color ? `ctp-toast--color-${item.color}` : '', { 'ctp-toast--exiting': item.exiting }, item.className || '']"
      :style="item.style"
      role="alert"
      aria-live="assertive"
    >
