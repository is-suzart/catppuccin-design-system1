...
if (args.length === 0 || args.includes('--help')) {
  console.log(`
  Usage: npx @catppuccin-ds/css compile [options]
  ...
  Examples:
    npx @catppuccin-ds/css compile --prefix=acme --output=src/styles/ds.css
    npx @catppuccin-ds/css compile --prefix= --output=dist/clean.css
  `);
  process.exit(0);
}
...
if (!fs.existsSync(srcPath)) {
  console.error('Source CSS not found. Is @catppuccin-ds/css installed?');
  process.exit(1);
}
