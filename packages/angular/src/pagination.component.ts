  ...
  template: `
    <nav [class]="containerClass()" aria-label="Pagination">
      ...
      @for (item of range(); track $index) {
        @if (item === DOTS) {
          <span class="ctp-pagination-ellipsis">{{ DOTS }}</span>
        } @else {
          <button
            class="ctp-pagination-item"
            [class]="'ctp-pagination-item--' + size()"
            [class.ctp-pagination-item--active]="item === currentPage()"
...
export class PaginationComponent {
  ...
  protected containerClass = computed(() => {
    return [
      'ctp-pagination',
      `ctp-pagination--${this.size()}`,
      `ctp-pagination--${this.shape()}`,
      `ctp-pagination--${this.color()}`,
    ].filter(Boolean).join(' ');
  });
