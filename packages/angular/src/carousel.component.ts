  ...
  template: `
    <div class="ctp-carousel" role="region" aria-label="Carousel">
      <div class="ctp-carousel-viewport">
        <div class="ctp-carousel-track" [style.transform]="'translateX(-' + current() * 100 + '%)'">
          @for (slide of slides; track $index) {
            <div class="ctp-carousel-slide" role="group" aria-roledescription="slide" [attr.aria-label]="'Slide ' + ($index + 1) + ' of ' + slides.length">
              <ng-content />
            </div>
          }
        </div>
      ...
      @if (showArrows() && slides.length > 1) {
        <button class="ctp-carousel-btn ctp-carousel-btn" data-state="prev" (click)="prev()" [disabled]="current() === 0" aria-label="Previous slide">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
            <polyline points="15 18 9 12 15 6"/>
          </svg>
        </button>
        <button class="ctp-carousel-btn ctp-carousel-btn" data-state="next" (click)="next()" [disabled]="current() === slides.length - 1" aria-label="Next slide">
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
      ...
      @if (showDots() && slides.length > 1) {
        <div class="ctp-carousel-dots" role="tablist" aria-label="Slides">
          @for (dot of slides; track $index) {
            <button
              class="ctp-carousel-dot" [attr.data-state]="$index === current() ? 'active' : null"
              (click)="goTo($index)"
              role="tab"
