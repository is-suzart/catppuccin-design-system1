...
@Component({
  ...
  standalone: true,
  template: `
    <div [class]="'ctp-pro-kanban-board ctp-pro-kanban-board--' + color() + ' ' + className()">
      @for (column of columns(); track column.id) {
        <div
          [class]="'ctp-pro-kanban-column ctp-pro-kanban-column--' + (column.color || color())"
        ...
        >
          @if (renderColumnHeader()) {
            ...
            <div class="ctp-pro-kanban-column-header">
              <div class="ctp-pro-kanban-column-title-container">
                <span class="ctp-pro-kanban-column-dot"></span>
                <h3 class="ctp-pro-kanban-column-title">{{ column.title }}</h3>
              </div>
              <span class="ctp-pro-kanban-column-badge">{{ getColumnItems(column.id).length }}</span>
            </div>
          }

          <div class="ctp-pro-kanban-cards-container">
            @for (item of getColumnItems(column.id); track item.id) {
              <div
                ...
                [class.ctp-pro-kanban-card--dragging]="draggedItem()?.id === item.id"
                class="ctp-pro-kanban-card"
                (click)="handleCardClick(item)"
              >
                @if (renderItem()) {
                  <div [innerHTML]="renderItem()!(item)"></div>
                } @else {
                  <div class="ctp-pro-kanban-card-title">{{ item.title }}</div>
                  @if (item.description) {
                    <div class="ctp-pro-kanban-card-desc">{{ item.description }}</div>
                  }
                  @if (item.tags && item.tags.length > 0) {
                    <div class="ctp-pro-kanban-card-tags">
                      @for (tag of item.tags; track tag) {
                        <span class="ctp-pro-kanban-card-tag">{{ tag }}</span>
                      }
                    </div>
                  }
                }
            ...
            @if (getColumnItems(column.id).length === 0) {
              <div class="ctp-pro-kanban-empty-column-placeholder">
                Solte itens aqui
              </div>
            }
          </div>
